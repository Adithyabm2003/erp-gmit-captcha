...
def check_audio_captcha():
    ...
    if ans.get() == get_audio():
        ...
        webbrowser.open_new("http://erp.gmit.info/axis/booking.html")
...
Title_Label = Label(Tops, font=('ALGERIAN', 18, 'bold'),
                    text="\t \tGM INSTITUTE OF TECHNOLOGY \t\t\t", bg='skyblue',
...
RightFrame = Frame(MainFrame, bd=4, width=200, height=200, padx=10, pady=2, bg='#68DEED', relief=RIDGE)
RightFrame.pack(side=RIGHT)

Label_1 = Label(RightFrame, font=('lato black', 33, 'bold'), text=" Audio Captcha ", padx=2, pady=2, bg="POWDER BLUE",
                fg="black")
...
def generate_image_captcha():
    # os.startfile('c1.png')
    captchaimg = PhotoImage(file="c1.png")
    captchabutton = Label(LeftFrame, image=captchaimg)
    captchabutton.place(x=80, y=65)
...
Label_1 = Label(LeftFrame, font=('lato black', 33, 'bold'), text=" Image Captcha ", padx=2, pady=2, bg="POWDER BLUE",
                fg="black")
Label_1.grid(row=0, column=0, sticky=W)
...
# label_19=Label(LeftFrame, image=c1.png)
# label_19.grid(row=2,coloumn=2)
